...
<section id="operations" class="panel hidden"><div class="operation-grid"><label for="operationSelect">Operation</label><select id="operationSelect"></select><span id="operationNote" class="note"></span><span>Working directory</span><code id="operationCwd"></code><span class="note">The directory containing input A.</span></div><div id="operationFields" class="operation-grid" style="margin-top:12px"></div><textarea id="commandPreview" class="command" readonly spellcheck="false" aria-label="Operation command preview"></textarea><div class="controls"><button id="copyOperation">Copy command</button><button id="runOperation" class="run">Run</button><button id="stopOperation" class="stop">Stop</button><button id="clearOperation">Clear output</button><span id="operationStatus" class="note">Idle</span></div><pre id="operationOutput" class="preview operation-output"></pre></section>
<section id="visualization" class="panel hidden"><div class="viz-layout"><div><div class="operation-grid"><label for="vizSelect">Plot utility</label><select id="vizSelect"></select><span id="vizNote" class="note"></span><span>Working directory</span><code id="vizCwd"></code><span class="note">The directory containing input A.</span></div><div id="vizFields" class="operation-grid" style="margin-top:12px"></div><textarea id="vizPreview" class="command" readonly spellcheck="false" aria-label="Plot command preview"></textarea><div class="controls"><button id="copyViz">Copy command</button><button id="runViz" class="run">Generate</button><button id="stopViz" class="stop">Stop</button><span id="vizStatus" class="note">Idle</span></div><pre id="vizOutput" class="preview viz-log"></pre></div><div><div class="controls"><button id="previousViz">Previous</button><button id="nextViz">Next</button><button id="standaloneViz">Open standalone</button><button id="downloadViz">Export .mplfig</button><span id="vizImageStatus" class="note">No captured figure</span></div><img id="vizImage" class="viz-image"></div></div></section>
<script>
let state, currentProfile, operationTimer, vizTimer, vizIndex=0, vizCount=0, vizFigureCount=0, vizFigureAvailable=[];
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function copyCommand(id){let field=document.getElementById(id);field.focus();field.select();try{await navigator.clipboard.writeText(field.value)}catch(_error){document.execCommand('copy')}}
